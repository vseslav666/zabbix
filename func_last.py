...
#функция переводящая время работ в проценты от месяца
...
#Функция проверяющая наличее файла с временем сбоя
...
def create_table_report(company):
    if company == 17:
        filename = ('Энергосбыт волга отчет  по SLA ' + str(datetime.date.today()) + '.xlsx')
    elif company == 18:
        filename = ('Северная_сбытовая_компания_отчет_по_SLA ' + str(datetime.date.today()) + '.xlsx')
    z = ZabbixAPI('http://localhost/zabbix/')
